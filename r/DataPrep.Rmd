...
idx$cf[1,2]/(idx$cf[1,2] + idx$cf[2,2]) # FN Rate: 0.4890068
idx$cf[2,2]/(idx$cf[2,2] + idx$cf[2,1]) # Precision: 0.9732852
idx$cf[2,2]/(idx$cf[2,2] + idx$cf[1,2]) # Recall: 0.5109932
(2 * idx$cf[2,2]) / ((2 * idx$cf[2,2]) + idx$cf[2,1] + idx$cf[1,2]) # F1-Score: 0.5109932


d.model <- glm(CRITICAL ~ EVENT_TYPE_CD + ALARM_TO_FD + RESPONSE_TYPE + AID_TO_FROM_OTHER_DEPTS + EST_KM + INITIAL_UNIT_PERSONNEL + INCIDENT_DAY + MONTH + YEAR, family=binomial(link='logit'), data = I[idx$train,])
y <- data.frame(prob=vector(mode='numeric',length=length(idx$cv)))
...
### KNN 
Doesn't work because of categorical variables (duh?)
Maybe Naive Bayes or Decision Tree
```{r}
# library(class)
d.model.knn <- knn(
  train=I[idx$train, c('EVENT_TYPE_CD','ALARM_TO_FD','RESPONSE_TYPE','AID_TO_FROM_OTHER_DEPTS','EST_KM','INITIAL_UNIT_PERSONNEL','INCIDENT_DAY','MONTH','YEAR')],
  test=I[idx$cv, c('EVENT_TYPE_CD','ALARM_TO_FD','RESPONSE_TYPE','AID_TO_FROM_OTHER_DEPTS','EST_KM','INITIAL_UNIT_PERSONNEL','INCIDENT_DAY','MONTH','YEAR')],
  cl=as.numeric(I[idx$train, 'CRITICAL']),
  k=3
)
```
...
nb$cf[1,2]/(nb$cf[1,2] +nb$cf[2,2]) # FN Rate: 0.2892343
nb$cf[2,2]/(nb$cf[2,2] +nb$cf[2,1]) # Precision: 0.8434
nb$cf[2,2]/(nb$cf[2,2] +nb$cf[1,2]) # Recall: 0.710
(2 *nb$cf[2,2]) / ((2 *nb$cf[2,2]) +nb$cf[2,1] +nb$cf[1,2]) # F1-Score: 0.7714462
