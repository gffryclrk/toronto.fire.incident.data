...
#   if(!is.na(x$INITIAL_CALL_HOUR) & !is.na(x$ONSCENE_HOUR)){
#     # if(x$ONSCENE_HOUR < x$INITIAL_CALL_HOUR){
#       # ic_time <- paste(x$INITIAL_CALL_HOUR, x$INITIAL_CALL_MIN, x$INITIAL_CALL_SEC, sep=" ")
#       ic_time <- paste(as.Date(x$INCIDENT_DATE), " ",formatC(x$INITIAL_CALL_HOUR, width=2, flag="0"), ":", formatC(x$INITIAL_CALL_MIN, width=2,flag="0"), ":",formatC(x$INITIAL_CALL_SEC, width=2, flag="0"), sep="")
...
#       # os <- as.POSIXct()
#       # cat(os_time, ic_time)
#       return(as.POSIXct(os_time, format="%Y-%m-%d %H:%M:%S") - as.POSIXct(ic_time, format="%Y-%m-%d %H:%M:%S"))
#     # }
# 
#    
#   # }else{
...
# This function works if you pass it a single row from the data.frame, such as I[1695,]
# However, there are problems if you try to run it against the entire dataset. 
# tta <- function(x){
#   onscene_date <- x$INCIDENT_DATE
#   if(x$ONSCENE_HOUR < x$INITIAL_CALL_HOUR){ onscene_date <- as.Date(x$INCIDENT_DATE) + 1 }
#   ic_time <- paste(as.Date(x$INCIDENT_DATE), " ",formatC(x$INITIAL_CALL_HOUR, width=2, flag="0"), ":", formatC(x$INITIAL_CALL_MIN, width=2,flag="0"), ":",formatC(x$INITIAL_CALL_SEC, width=2, flag="0"), sep="")
#   os_time <- paste(as.Date(onscene_date)," ",formatC(x$ONSCENE_HOUR, width=2, flag="0"), ":", formatC(x$ONSCENE_MIN, width=2,flag="0"), ":",formatC(x$ONSCENE_SEC, width=2, flag="0"), sep="")
#   # cat(ic_time,"\n",os_time,"\n")
#   return(as.POSIXct(os_time, format="%Y-%m-%d %H:%M:%S") - as.POSIXct(ic_time, format="%Y-%m-%d %H:%M:%S"))
# }
...
tta <- function(x){
  ...
  onscene_date <- as.Date(x["INCIDENT_DATE"])
  if(x["ONSCENE_HOUR"] < x["INITIAL_CALL_HOUR"]){ onscene_date <- as.Date(x["INCIDENT_DATE"]) + 1 }
  ic_time <- paste(as.Date(x["INCIDENT_DATE"]), " ",formatC(x["INITIAL_CALL_HOUR"], width=2, flag="0"), ":", formatC(x["INITIAL_CALL_MIN"], width=2,flag="0"), ":",formatC(x["INITIAL_CALL_SEC"], width=2, flag="0"), sep="")
  os_time <- paste(as.Date(onscene_date)," ",formatC(x["ONSCENE_HOUR"], width=2, flag="0"), ":", formatC(x["ONSCENE_MIN"], width=2,flag="0"), ":",formatC(x["ONSCENE_SEC"], width=2, flag="0"), sep="")
  # cat(ic_time,"\n",os_time,"\n")
...
# I$TTA <- apply(I, 1, tta) # This is how I obtained the TTA info... runtime: about 20 minutes
...
sum(testColumn(I[1:nrow(I),]), na.rm=T) #563070
sum(is.na(testColumn(I[1:nrow(I),]))) #17275
