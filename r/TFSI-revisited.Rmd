...
Now that we have a few metrics we can visualize and compare them.

```{r}
model_log_imb_predict$cls <- as.numeric(I_s[full_idx$cv, 'CRITICAL']) -1
model_log_imb_predict$acc <- ((model_log_imb_predict$pred & model_log_imb_predict$cls) | (!model_log_imb_predict$pred & !model_log_imb_predict$cls))
custom.summaries(table(model_log_imb_predict$pred, model_log_imb_predict$cls))
mean(model_log_imb_predict$acc, na.rm=T) # ACCURACY
